...
import { Button } from "@/components/ui/button";
import { Loader2, Edit3, Trash2, User } from "lucide-react";
import axios from "axios";
import { toast } from "sonner";
import { API_BASE_URL, API_ENDPOINTS, apiUrl } from "@/lib/api";

export interface Candidat {
  id_candidat: number; // Clé primaire stricte Django
  nom_cand: string;
  pren_cand: string;
  genre: string;
  dat_nais: string;
...
export default function CandidatList({ onAdd, onEdit }: CandidatListProps) {
  ...
  const fetchCandidats = useCallback(async () => {
    ...
    try {
      ...
      setCandidats(Array.isArray(res.data) ? res.data : res.data.results ?? []);
    } catch {
  ...
  const handleDelete = async (id_candidat: number) => {
    if (!confirm("Supprimer ce candidat définitivement ?")) return;
    try {
      await axios.delete(`${API_URL}${id_candidat}/`, getAuthHeaders());
      toast.success("Candidat supprimé");
      fetchCandidats();
    } catch {
      toast.error("Échec de la suppression");
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-bold text-[#0A5C36]">Profils Candidats ({candidats.length})</h2>
        <Button onClick={onAdd} className="bg-[#0A5C36] text-white hover:bg-[#1B7A53]">
          + Ajouter
        </Button>
      </div>

      <div className="rounded-2xl border border-[#E6F4ED] overflow-hidden bg-white">
        <Table>
          <TableHeader>
            <TableRow className="bg-[#E7F5EF]">
              ...
              candidats.map((c) => (
                <TableRow key={c.id_candidat} className="hover:bg-[#F3F9F5]">
                  <TableCell className="font-medium flex items-center gap-2">
                    {c.photo ? (
                      <img
                        src={c.photo.startsWith("http") ? c.photo : `${API_BASE_URL}${c.photo}`}
                        alt=""
                        className="w-8 h-8 rounded-full object-cover"
                      />
                    ) : (
                      <div className="w-8 h-8 rounded-full bg-[#E7F5EF] flex items-center justify-center">
