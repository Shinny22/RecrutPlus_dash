...
export default function CandidatForm({ onAdded, onCancel, editId }: CandidatFormProps) {
  ...
  useEffect(() => {
    ...
    // 1. Charger les diplômes
    fetch(apiUrl(API_ENDPOINTS.diplomes), { headers })
      .then(res => res.json())
      .then(data => setDiplomes(data))
      .catch(err => console.error("Erreur diplômes:", err));
    ...
    if (editId) {
      setLoadingInitial(true);
      fetch(apiUrl(`${API_ENDPOINTS.candidats}${editId}/`), { headers })
        .then(res => res.json())
        .then(data => {
          setInitialValues({
            nom_cand: data.nom_cand || "",
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      // 🔑 Récupération du token pour la soumission FormData
      const token = localStorage.getItem("access_token");
